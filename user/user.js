...
const findUsers = function* () {
    const ctx = this;
    const query = Object.assign({page: 0, limit: 10, orderBy: 'experience', order: -1}, ctx.query);
    const match = {};

    // 根据关注或粉丝
    if (query.search && ['fans', 'followings'].indexOf(query.search) !== -1) {
        match[query.search] = {$in: [new ObjectId(query.keyword)]};
    }
    ...
    if (query.search && query.search === 'nickname') {
        ...
        match[query.search] = {$regex: query.keyword};
    }
    ...
    try {
        const list = yield User.aggregate()
            ...
            .project({
                nickname: 1,
                avatar: 1,
                collectionsNumber: {$size: '$collections'},
                sharesNumber: {$size: '$shares'},
                fansNumber: {$size: '$fans'},
                followingsNumber: {$size: '$followings'}
            })
            .exec();
...
const findSharesByMe = function* () {
    ...
    const project = {
        shares: {$slice: [query.limit * query.page, query.limit * query.page + query.limit * 1]}
    };

    try {
        const user = yield User.findOne({_id: new ObjectId(_id)}, project)
            .populate('shares');
...
const findSharesBycollections = function* () {
    ...
    const query = ctx.query;
    const project = {
        collections: {$slice: [query.limit * query.page, query.limit * query.page + query.limit * 1]}
    };
    ...
    try {
        const promises = [
            User.findByIdAndUpdate(_id, {
                $push: {fans: {$each: [field.userId], $position: 0}}
            }),
            User.findByIdAndUpdate(field.userId, {
                $push: {followings: {$each: [_id], $position: 0}}
            })
        ];
        yield promises;
...
const judgefollowUser = function* () {
    ...
    const _id = ctx.params._id;
    try {
        const user = yield User.find({_id: _id, fans: {$in: [userId]}});
...
router.get('/:_id/judgefollow', passport.authenticate('jwt', {session: false}), judgefollowUser);

router.get('/:_id/share', passport.authenticate('jwt', {session: false}), findSharesByMe);
router.get('/:_id/share/collect', passport.authenticate('jwt', {session: false}), findSharesBycollections);


router.put('/:_id', passport.authenticate('jwt', {session: false}), updateUser);
router.put('/:_id/follow', passport.authenticate('jwt', {session: false}), followUser);
router.put('/:_id/unfollow', passport.authenticate('jwt', {session: false}), unfollowUser);
