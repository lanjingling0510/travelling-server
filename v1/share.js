...
const createShare = function* () {
  ...
  try {
    ...
    //  修改user集合
    yield User.findByIdAndUpdate(field.userId, {
      $push: {
        shares: {
          $each: [new ObjectId(share._id)],
          $position: 0
        }
      }
    });
...
const removeShare = function* () {
  ...
  try {
    ...
    const promise2 = [
      ...
      User.findByIdAndUpdate(share.userId, {
        $pull: {
          shares: _id
        }
      }), //  从发布者的分享列表中清除
...
router.put('/:_id/collect', passport.authenticate('jwt', {
  session: false
}), collectShare);
router.put('/:_id/uncollect', passport.authenticate('jwt', {
  session: false
}), uncollectShare);
router.del('/:_id', passport.authenticate('jwt', {
  session: false
}), removeShare);
